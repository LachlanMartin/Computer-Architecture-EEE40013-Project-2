...
bool exprnx(const char *&ptr, int64_t &value) {
   if (pass == 1) {
 ...
 **
 */
void print_line(op_entry *opEntry, FILE *ofile) {
   ...
   // Disassemble
   if (!err_flag && (mnemonic != nullptr) && debug) {
      ...
      switch(opcode&(0b111<<29)) {
         ...
         case 1U<<29: // R,R,#
               ...
               if ((opEntry == nullptr)||(opEntry->ea_mask == SIGNED_IMM)) {
                  fprintf(ofile,"; %-5s R%-2d,R%-2d,#%i",
                        getAluOpName((opcode>>26)&0b111),
                        (opcode>>21)&0b11111,
                        (opcode>>16)&0b11111,
...
enum {
   ...
   ERR_OP_TOO_LARGE,
   ERR_ILL_SIZE,
   ERR_UNSIGNEDVAL_OUT_OF_RANGE,
   ERR_SIGNEDVAL_OUT_OF_RANGE,
...
static void asm_error(unsigned err_num) {
   ...
   // Only print label errors in 1st pass and all in 2nd pass
   bool printError = (pass == 2) ||
                     (err_num == ERR_LABEL_MULTIPLY_DEFINED) ||
                     (err_num == ERR_ILLEGAL_LABEL) ||
                     (err_num == ERR_LABEL_NOT_ALLOWED) ||
                     (err_num == ERR_LABEL_REQUIRED);
   if (err_num > MAX_ERROR_MESSAGE) {
      err_num = 0;
   }
   if (!err_flag) /* only report first error on each line */
   {
      /* only flag 1 error but multiply warnings */
      err_flag = 1;
      if (printError) {
         ...
         fprintf(listfile,"E***** : %s\n", err_messages[err_num]);
         if (pass==1) {
            /* print out offending line in pass 1 */
            print_line(nullptr, listfile);
         }
         print_line(nullptr, stderr);
      }
   }
}
...
static bool isS16Size( int64_t value ) {

   return value == (int64_t)((int16_t) value);
}

static bool isU16Size( int64_t value ) {

   return value == (int64_t)((uint16_t) value);
}
...
int do_INDEXED(op_entry &) {
   ...
   if (!isS16Size(value)) {
      asm_error(ERR_SIGNEDVAL_OUT_OF_RANGE);
      return(0);
   }
...
constexpr uint32_t AddImmediateOpcode = 0x00000000;
constexpr uint32_t SubImmediateOpcode = 0x04000000;

/*
  <mnemonic> Ra,Rb
  <mnemonic> Ra,#dddd
 */
int do_2ADDRESS(op_entry &opEntry) {
   ...
   if (parse_ea( argptr, "%21R,%11R", opcode, value )) { // Ra,Rb
      ...
      if ((entry->opcode == AddImmediateOpcode) && (value < 0)) {
         // add -ve => sub +ve
         opcode = (opcode & 0xE3FFFFFF) | SubImmediateOpcode;
         value = -value;
      }
      if ((entry->opcode == SubImmediateOpcode) && (value < 0)) {
         // sub -ve => add +ve
         opcode = (opcode & 0xE3FFFFFF) | AddImmediateOpcode;
         value = -value;
      }
      if (opEntry.ea_mask == SIGNED_IMM) {
         if (!isS16Size(value)) {
      ...
      else if (opEntry.ea_mask == UNSIGNED_IMM) {
         if (!isU16Size(value)) {
            asm_error(ERR_UNSIGNEDVAL_OUT_OF_RANGE);
            return(0);
         }
      }
      // # immediate
...
/*
  ...
  <mnemonic> Ra,Rb,#dddd
 */
int do_2or3ADDRESS(op_entry &opEntry) {
   int64_t value = 0;
...
int report_error_count(void) {
   ...
   if (err_pass1>0) {
      fprintf(listfile,"Pass 2 not done\n");
   }
   else {
...
int assem1(char *line) {
   ...
   if (entry != NULL) {/* found in mnemonic table ? */
      ...
      instrn_length = entry->clazz(*entry); /* assemble line */
   }
   else {
      if ((label != NULL) &&                 /* label and */
            !enter_symbol(label,initial_pc,seg_type[current_segment]))
...
int assem2(char *line) {
   ...
   if (entry != NULL) /* found in mnemonic table ? */
   {
      ...
      instrn_length = entry->clazz(*entry);
   }
   else
      asm_error(ERR_UNKNOWN_MNEMONIC);

   print_line(entry, listfile);

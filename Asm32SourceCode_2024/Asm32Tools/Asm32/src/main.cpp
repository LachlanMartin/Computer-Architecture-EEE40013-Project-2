...
int  fparts;
  ...
  while (--argc > 0) /* process each argument */
   ...
   case 'q' :  /* quiet - no banner */
       quiet = 1;
       break;
   case 'd' :  /* disassemble generated instructions */
       debug = 1;
       break;
	default :
...
int pass2(void) {
   ...
   int err_count = report_error_count();
...
//     const char *ptr = tests[index];
//     int64_t result;
//     int ok = exprn(ptr, result);
//     printf("%-20s => %ld (0x%lX) %s\n", tests[index], result&0xFFFFL, result&0xFFFFL, ok?"OK":"Failed");
//  }

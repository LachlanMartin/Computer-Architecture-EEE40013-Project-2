...
void print_symbol_table(FILE *lstfile) {
   ...
   for (symbol_ptr = symbol_table; symbol_ptr->name != NULL; symbol_ptr++)
   {
      ...
      if (symbol_ptr->type & EXTERN_SYM) /* extern */
         strcat(type,"_E");
      fprintf(lstfile,"%8.8lX : %5.5s : %s\n",
            symbol_ptr->value&0xFFFFFFFFL,
            type,
            symbol_ptr->name
      );
   }
 ...
 */
int enter_symbol(const char *name, int64_t value, entry_type type) {
   sym_entry *symbol_ptr;
 ...
 */
bool symbol_value(const char *name, int64_t &value) {
   sym_entry *symbol_ptr;
